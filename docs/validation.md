...
最終更新日: 2026-09-20（初期実装の検証: 2026-09-08）
...
## 2026年9月20日：ChatGPT会話の複数回取り込み

Node.js 24.21.0、npm 11.19.0、Playwright Chromium 153.0.8010.12で型チェック、単体・Profile・fixture 167件、ブラウザ16件が成功しました。ブラウザ本体と必要なOSライブラリが環境に未導入だったため、Playwrightのインストール手順で追加してから検証しました。

単体テストでは重複・逆順・同じ本文の別発言、複数メッセージを含むターン、表示用DOM属性だけの差、欠番の解消、0始まり・途中開始・巨大な番号間隔、識別情報不足、空本文、本文・ID・話者の競合、入力全体の拒否と既存状態の保持、リセットを検証しています。8ファイルの取得範囲を再現した合成データも回帰テストに含めています。提供HTMLの本文やIDはコミットしていません。

ブラウザテストではペースト直後の追加と出力、欠落前後の本文表示、欠番の解消、Markdownだけのコピー、失敗時の蓄積保持、手入力の追加、入力方式・取り込み方法の切り替え、別会話へのリセット、入力由来HTMLの非実行と外部通信・永続保存がないことを確認しました。

提供された `tmp/chatgpt_20260920_in-01.html` 〜 `in-08.html` 全体をNode.jsと配布HTMLの両方で取り込み、次の結果を確認しました。

| 項目 | 結果 |
| --- | --- |
| 延べターン数 / 重複除去 / 取得件数 | 75 / 53 / 22 |
| 取得範囲 | 1〜14、16〜18、28〜32 |
| 未取得範囲 | 15、19〜27 |
| Markdown | 事前の検証用結合Markdownと完全一致 |
| Node.jsでの逆順・再投入 | 出力不変、再投入の新規件数0 |
| ブラウザでの8回のペースト | 全件成功。1回あたり約234〜300ms（この環境での参考値） |
| ブラウザの外部要求 / ページエラー | 0 / 0 |

1360px幅の画面を目視確認し、720px幅ではページの横はみ出しがないことも確認しました。出力と画面記録はGit管理対象外の `tmp/chatgpt_20260920_collected.md`、`tmp/chatgpt-collection-desktop.png`、`tmp/chatgpt-collection-narrow.png` に保存しています。OSの実クリップボードによるChrome・Edgeの操作確認は含みません。
