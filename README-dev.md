...
ChatGPT会話については、単発変換に加えて複数断片の蓄積・重複除去と取得範囲の表示に対応しています。これは取り込み方法の追加であり、Extraction ProfileやMarkdownの出力形式は共通です。
...
取り込み方法は「1回で変換」、入力方式は「リッチコピー」、変換モードはAutoを初期値とします。単発変換では入力欄の編集、受け付けた貼り付け、入力方式・変換モードの変更時に前回の出力・warning・エラー・検出結果をクリアし、Convertの実行を待ちます。入力方式の変更時だけ入力もクリアします。受理できない貼り付けでは既存の入力・出力を保持し、別の入力エラー表示領域に理由を示します。
...
`src/core/chatgpt-collection.ts` の `createChatgptCollection()` がページ内の1会話を管理します。`add(html)` は新規・重複件数とスナップショット、または入力を拒否した理由を返します。`reset()` で蓄積を破棄します。スナップショットには取得件数、連続する取得番号の範囲、欠番範囲と前後の本文冒頭、先頭欠落の可能性、既存の `ConvertResult` の成功結果を含めます。

* HTMLは既存の不活性parserとsanitizeを通し、最外側の `conversation-turn-N` 要素を取り出します。番号は0以上の安全な整数に限ります。
* 各ターンには著者要素と空でない `data-message-id` が必要です。1ターン内の複数メッセージは全IDをDOM順で保持します。番号・ID不足、空本文、未知または矛盾する話者は拒否します。
* 同じ番号では全IDの並びと既存converterによるMarkdownの完全一致を確認し、重複として扱います。同じ本文でも異なる番号・IDなら別発言として保持します。装飾用DOM属性の違いはMarkdownに影響しなければ競合になりません。
* 同じ番号でIDが異なる、同じIDが別番号に現れる、同じIDで本文が異なる場合は競合としてその入力全体を拒否します。本文の長さで勝手に選択したり、部分本文を推測で接合したりしません。空要素が混ざる場合も、その入力全体を取り直します。
* 全ターンを番号順に並べて既存converterへ渡し、結合後の範囲で完全性の警告を再計算します。全検査と結合変換が成功した後にだけ状態を確定します。欠番は範囲で保持し、大きな番号間隔でも番号を列挙しません。
* 会話IDを推測で取得せず、利用者が同じ会話だけを追加することを前提とします。番号・IDが重ならない別会話の混入は検出できません。単一ターン本文の完全性と、未取得の末尾も保証しません。

ブラウザではこの取り込み方法に切り替えるとChatGPT Profileに固定し、受理したペーストを即時追加します。手入力・編集時は蓄積結果を保持して「入力を追加」を待ちます。変換に失敗した入力も確認用に入力欄へ表示しますが、蓄積結果とコピー機能は保持します。入力方式を変えても蓄積は残ります。取り込み方法を切り替えると入力欄をクリアし、単発変換のモードを復元します。蓄積した会話はリセットまたはページ終了まで保持し、複数回取り込みへ戻ると再表示します。状態・入力・出力は永続保存しません。

取得範囲と欠落を示すUIも `textContent` で構築し、入力由来のHTMLを挿入しません。コピー対象は結合したMarkdownだけです。範囲に欠番がない場合も、会話全体の取得完了とは表示しません。
