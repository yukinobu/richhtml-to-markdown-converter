...
| 段落・改行 | `p` とブロック要素は段落境界、`br` はバックスラッシュ＋LFのhard break。通常テキスト内の連続するHTML空白は半角スペース1個に畳み、段落の前後は除去 |
...
通常テキストではMarkdown構文として誤解されるASCII句読点をバックスラッシュでescapeします。初期出力を一意にするため、入力由来のテキストはコード・URL以外のASCII句読点（U+0021–002F、003A–0040、005B–0060、007B–007E）をすべてescapeします。Rendererが生成するMarkdown構文やfront matterはこの対象外です。装飾の内側にある前後の空白はdelimiterの外へ移し、コード本文はescapeしません。HTML文字参照はDOM解析で復号し、raw HTMLを出力へ残しません。

結合セルがなく、各行のセル数が同じで、セルがインライン内容だけの表をGFMのpipe tableへ変換します。最初の行がすべて `th` ならヘッダーとし、そうでなければ空のヘッダーを補って全行をデータ行にします。セルの両側にはスペース1個、区切り行は `---`、列の配置指定は出力しません。セル内の `br` はスペースへ置換し、`|` はコード内も含め `\|` とします。`caption` は表の直前の段落として残します。
